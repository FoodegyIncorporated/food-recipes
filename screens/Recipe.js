...
export default function Recipe({ route }) {
	...
	function fetchDistances(data, targetUrl) {
		...
		for (var i = 0; i < tempUrls.length; i++) {
			...
			let urlParameters = Object.entries(raw)
				.map((e) => e.join('='))
				.join('&');
	...
	const listIngredients = ingredients.map((i) => (
		<Text style={styles.ingredients} key={i.original}>
			{i.original}
		</Text>
	));
	...
	const listSteps = steps.map((i) =>
		i.steps.map((j) => (
			<Text style={styles.stepNum} key={j.number}>
				{j.number + ') '}
				<Text style={styles.stepInfo} key={j.step}>
					{j.step}
				</Text>
			</Text>
		))
	...
	const listSimilarRecipes = similarRecipes
		.filter((x) => x.title !== title)
		.map((i) => (
			<TouchableOpacity
				onPress={() => renderSimilarRecipe(i.id)}
				style={styles.similarRecipesContainer}
				key={i.id}
			>
				<Text style={styles.similarRecipes} key={i.id}>
					{i.title}
				</Text>
			</TouchableOpacity>
		));
		...
		>
			...
			<View style={styles.content}>
				<View style={styles.contentContainer}>
					...
					<View style={{ padding: 5 }}>{listIngredients}</View>
				</View>
			</View>
			<View style={styles.content}>
				<View style={styles.contentContainer}>
					...
					<View style={{ padding: 5 }}>
						{steps.length !== 0 && !loading ? (
							...
							<Text style={styles.noSteps}>No steps found.</Text>
						)}
					</View>
				</View>
			</View>
